...
- **产品 / 策划 / 个人开发者** —— 和 AI 头脑风暴把功能聊透(有设计案就喂给它、没有就靠访谈)→ **可点原型**试玩 → 迭代 → **定稿包,可交另一个 AI 直接开发**。
- **程序** —— **类型化接口契约**(端 = 端同一份)+ 配置说明 + 验收用例 → 另一个 AI(或你自己)照着实现,无歧义。
- **测试 / QA** —— **验收用例 + 可视化 Excel 清单** → 另一个 AI 跑测试,或用 Excel 做黑盒测试。

![AIGD:和 AI 头脑风暴做设计,产出交接包,再用确定性校验门控](docs/session.gif)

**你只需做两件事:和 AI 头脑风暴讨论设计、把数值配好。** 其余交给流程——它把讨论沉淀成结构化产出(规则挂编号 · 数值住配置 · 界面 DSL · 接口契约 · 验收用例),用**确定性校验器**门控一致性,最后打包成一份**平台无关的交接包**,足以让**另一个 AI 直接照着落地实现**(本仓库已真跑验证 ↓)。
